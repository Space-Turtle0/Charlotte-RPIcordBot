...
class ColorUser(BaseModel):
    id = AutoField()
    user_id = BigIntegerField(unique=True)
    role_id = BigIntegerField(default="0")

class NameToDiscord(BaseModel):
    """
    # NameToDiscord:
    Name to Discord ID.
...
def initialize_db():
    db.connect()
    db.create_tables([ColorUser], safe=True)

# Function to get or create a user record
def get_or_create_user(user_id):
    user = ColorUser.create(user_id=user_id, role_id="0")
    user.save()
    return user

# Function to update the user's role ID
def update_user_role(user_id, role_id):
    ColorUser.update(role_id=role_id).where(ColorUser.user_id == user_id).execute()

# Function to find a user by their ID
def find_user_by_id(user_id):
    try:
        return ColorUser.get(ColorUser.user_id == user_id)
    except ColorUser.DoesNotExist:
        return None
